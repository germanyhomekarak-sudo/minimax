...
ملاحظات: ${data.notes || '—'}`;
  ...
  const init = async () => {
    ...
    /* ========== التحديث الفوري من لوحة المطور ========== */
    // دالة تحديث المنتجات فورياً
    window.__alBurjiRefresh = () => {
      try {
        const raw = localStorage.getItem('alburji.dev.state');
        if (!raw) return;
        const s = JSON.parse(raw);
        if (s.products?.products) {
          renderProducts(s.products);
        }
      } catch (e) {}
    };

    // دالة تحديث النصوص فورياً
    window.__alBurjiApplyContent = () => {
      try {
        const raw = localStorage.getItem('alburji.dev.state');
        if (!raw) return;
        const s = JSON.parse(raw);
        ...
        // Hero
        if (s.content['hero.title']) {
          const el = document.querySelector('[data-c="tagline"]');
          if (el) el.textContent = s.content['hero.title'];
        }
        if (s.content['hero.subtitle']) {
          const el = document.querySelector('.hero__sub');
          if (el) el.textContent = s.content['hero.subtitle'];
        }
        // About
        const ps = document.querySelectorAll('.about__text p');
        if (ps[0] && s.content['about.p1']) ps[0].textContent = s.content['about.p1'];
        if (ps[1] && s.content['about.p2']) ps[1].textContent = s.content['about.p2'];
        // Contact
        if (s.contact) fillFromData(s.contact);
      } catch (e) {}
    };

    // تطبيق الثيم والنصوص المخزّنة عند التحميل (إن وُجدت تعديلات محفوظة)
    try {
      const raw = localStorage.getItem('alburji.dev.state');
      if (raw) {
        const s = JSON.parse(raw);
